...
class ContentContainer {
  ...
  removeElement(element) {
    ...
    if (index !== -1) {
      this.elements.splice(index, 1); // Використовуємо splice для видалення елемента
    }
  }
}
...
class Message extends ContentContainer {
  ...
  constructor(content) {
    super(); // Виклик конструктора батьківського класу ContentContainer
    this.content = content;
  }
  // Створюємо метод display, який виводить ${this.content} для всіх елементів масиву
  display() {
    console.log(`${this.content}`);
    this.elements.forEach((element) => element.display(element));
  }
}
